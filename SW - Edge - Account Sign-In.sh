...
# the email address signed in.

# Since jq is now included by default on macOS we can use that more effectively than
# trying to parse the JSON with awk.
...
currentUser=$(/usr/bin/stat -f%Su "/dev/console") # u446852
lastUser=$(defaults read /Library/Preferences/com.apple.loginwindow lastUserName)

if [[ "$currentUser" = "" || "$currentUser" = "root" ]]
    then userHome=$(/usr/bin/dscl . -read /Users/"$lastUser" NFSHomeDirectory | awk -F ": " '{print $2}')
    else userHome=$(/usr/bin/dscl . -read /Users/"$currentUser" NFSHomeDirectory | awk -F ": " '{print $2}') # /Users/u446852
fi
